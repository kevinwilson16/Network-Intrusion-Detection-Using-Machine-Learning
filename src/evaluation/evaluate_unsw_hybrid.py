...
def main():
    ...
    try:
        iso_forest = joblib.load(os.path.join(
            MODELS_PATH, "iso_forest_unsw.pkl"))
    except Exception as e:
    ...
    try:
        xgb_model = joblib.load(os.path.join(
            MODELS_PATH, "xgb_unsw_smote.pkl"))
    ...
    report_hyb = classification_report(y_test_enc, final_hybrid_preds, labels=valid_classes,
                                       target_names=filtered_target_names, output_dict=True, zero_division=0)
